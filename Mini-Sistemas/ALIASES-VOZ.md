...
> **Objetivo:** Atalhos de comando para interações rápidas com IA em modo voz  
> **Criado:** 16/11/2025  
...
→ Acessa transcrições + extrai insights semanais
```
...
→ Ambiente criativo noturno + FinanDEV + criar arquivos novos
```
...
| Alias | Ação | Descrição |
|-------|------|-----------|
| `resume-hoje` | Resumo do dia | Commits + transcrições + decisões |
| `resume-semana` | Resumo semanal | Learnings + padrões + estatísticas |
| `resume-mes` | Resumo mensal | Metas atingidas vs. planejadas |

### Git e Commits
| Alias | Ação | Descrição |
|-------|------|-----------|
| `commit-urgente` | Git add + commit + push | Mensagem auto-gerada pelo contexto |
...
| `registra-sono` | SAUDE-MENTAL | Horas + qualidade sono |
| `humor-hoje` | SAUDE-MENTAL | Neutro/Positivo/Negativo + nota |
...
→ Abre FinanDEV + modo refatoração + analisa estrutura
```
...
- modo-doc → Cria/atualiza documentação
...
...
**Tempo economizado:** ~45min (vs. comandos completos)
```
